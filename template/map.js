...
function initMap(config) {
    ...
    config.chapters.forEach((record, idx) => {
        ...
        if (record.title) {
            var title_spacing = document.createElement('br');
            var title = document.createElement('h3');
            title.innerText = record.title;
            chapter.appendChild(title_spacing);
            chapter.appendChild(title_spacing);
        ...
        // Customizes the chapter 
        if (record.chapterAttributes) {
            for (const [attribute, value] of Object.entries(record.chapterAttributes)) {
                chapter.setAttribute(attribute, value);
            }
        }
        ...
        chapter.classList.add(record.theme);
    ...
    map.on("load", function () {
        ...
        let activeInterval;
        function toggleLayerVisibility(layers) {
            let currentLayerIndex = 0;
            return function() {
                const previousLayer = layers[(currentLayerIndex - 1 + layers.length) % layers.length];
                const currentLayer = layers[currentLayerIndex];
                map.setPaintProperty(previousLayer.layer, 'fill-opacity', 0); // Adjust the property according to layer type
                map.setPaintProperty(currentLayer.layer, 'fill-opacity', currentLayer.opacity);
        ...
        scroller
            ...
            .onStepEnter(response => {
                ...
                if (chapter.animationLayers) {
                    const animateLayers = toggleLayerVisibility(chapter.animationLayers);
                    animateLayers(); // Initialize animation
                    activeInterval = setInterval(animateLayers, chapter.animationInterval);
                }
